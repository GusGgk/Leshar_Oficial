...
if(isset($_GET['id'])){ // verifica se o id foi passado via GET
    ...
    $stmt = $conexao->prepare("SELECT * FROM avaliacao WHERE id = ?"); // prepara a query statement
    $stmt->bind_param("i", $id); // "i" indica que o parâmetro é um inteiro
} else {
    $stmt = $conexao->prepare("SELECT * FROM avaliacao"); // prepara a query statement sem filtro
}
